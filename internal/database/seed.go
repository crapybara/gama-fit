...
func seedMacros() {
	_, _ = DB.Exec(`INSERT OR REPLACE INTO user_macros_final (id, calories, protein, carbs, fats) VALUES (1, 2500, 200, 300, 70)`)
}
...
func seedGoals(now time.Time) {
	...
	for i, g := range goals {
		...
		_, err := DB.Exec(
			`INSERT INTO goals (title, completed, claimed, reward, claimed_at) VALUES (?, ?, ?, ?, ?)`,
...
func seedCheckins(now time.Time) {
	...
	for _, d := range days {
		...
		_, err := DB.Exec(`INSERT OR IGNORE INTO checkins (checkin_date) VALUES (?)`, date)
		if err != nil {
...
func seedSleepLogs(rng *rand.Rand, now time.Time) {
	...
	for i := 0; i < 14; i++ {
		...
		_, err := DB.Exec(
			`INSERT OR REPLACE INTO sleep_logs (log_date, bedtime, waketime, quality, duration_mins, score)
			 VALUES (?, ?, ?, ?, ?, ?)`,
...
func seedFoodCatalog() {
	...
	for _, item := range items {
		_, err := DB.Exec(
			`INSERT OR IGNORE INTO food_catalog (name, calories, protein, carbs, fats) VALUES (?, ?, ?, ?, ?)`,
...
func seedMeals(rng *rand.Rand, now time.Time) {
	...
	for i := 0; i < 14; i++ {
		...
		for j := 0; j < 2; j++ {
			...
			_ = DB.QueryRow(`SELECT calories, protein, carbs, fats FROM food_catalog WHERE name = ?`, name).Scan(&cal, &pro, &carb, &fat)
			logTime := fmt.Sprintf("%02d:%02d:00", 8+j*5+rng.Intn(2), rng.Intn(60))
			_, err := DB.Exec(
				`INSERT INTO daily_meals (name, calories, protein, carbs, fats, log_date, log_time) VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
func seedWorkoutPlans() {
	...
	for _, p := range plans {
		_, err := DB.Exec(
			`INSERT INTO workout_plans (day_of_week, exercise_name, sets, reps) VALUES (?, ?, ?, ?)`,
...
func seedFreestyleLogs(rng *rand.Rand, now time.Time) {
	...
	insert := func(date time.Time, ex lift, intensity float64) {
		weight := ex.Base + (intensity * ex.Gain) + ((rng.Float64() - 0.5) * 4)
		if weight < ex.Base*0.7 {
			weight = ex.Base * 0.7
		}
		...
		_, err := DB.Exec(
			`INSERT INTO freestyle_logs (exercise_name, weight, reps, logged_date, logged_time) VALUES (?, ?, ?, ?, ?)`,
			ex.Name, weight, reps, date.Format("2006-01-02"), logTime,
		)
		if err != nil {
	...
	for monthsBack := 35; monthsBack >= 0; monthsBack-- {
		...
		for session := 0; session < 4; session++ {
			...
			ex := lifts[(monthsBack+session)%len(lifts)]
			intensity := ex.Shift + progress
			insert(date, ex, intensity)
		}
	}
...
func seedShopCatalog() {
	...
	for _, item := range items {
		_, err := DB.Exec(
			`INSERT OR IGNORE INTO shop_catalog (category, name, cost, owned) VALUES (?, ?, ?, 0)`,
