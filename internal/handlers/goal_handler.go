...
<div id="goal-{{.ID}}" class="flex items-center justify-between p-3 rounded-xl border transition-all {{if .Completed}}bg-zinc-900/80 border-app-yellow/30{{else}}bg-zinc-900/40 border-white/5 hover:bg-zinc-900/80{{end}}">
	<div class="flex items-center gap-3">
		...
		{{else}}
			<div hx-put="/api/goals/{{.ID}}/toggle" hx-target="#goals-container" hx-vals='js:{local_date: new Date().toISOString().split("T")[0]}' class="w-5 h-5 rounded border-2 border-zinc-600 flex items-center justify-center shrink-0 hover:border-app-yellow transition-colors cursor-pointer"></div>
		{{end}}
	...
	<div class="flex items-center gap-2">
		...
		{{else if and .Completed (not .Claimed)}}
			<button hx-put="/api/goals/{{.ID}}/claim" hx-target="#goals-container" hx-vals='js:{local_date: new Date().toISOString().split("T")[0]}' class="text-[10px] font-bold uppercase tracking-wider bg-app-pink text-white px-3 py-1.5 rounded-lg hover:bg-app-yellow hover:text-black transition-all">Claim {{.Reward}}</button>
		{{else}}
...
func renderGoals(w http.ResponseWriter, r *http.Request, userID int) {
	rows, err := database.DB.Query("SELECT id, title, completed, claimed, reward FROM goals WHERE user_id = $1 ORDER BY id ASC", userID)
	if err != nil {
	...
	_ = database.DB.QueryRow("SELECT total_coins FROM user_stats WHERE user_id = $1", userID).Scan(&data.TotalCoins)
...
func HandleGoals(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if title != "" {
			_, err := database.DB.Exec("INSERT INTO goals (user_id, title, reward) VALUES ($1, $2, $3)", userID, title, reward)
			if err != nil {
...
func HandleGoalActions(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 2 && parts[1] == "toggle" {
		_, _ = database.DB.Exec("UPDATE goals SET completed = CASE WHEN completed = 1 THEN 0 ELSE 1 END WHERE id = $1 AND user_id = $2", id, userID)
	} else if len(parts) == 2 && parts[1] == "claim" {
		var reward int
		localDate, _ := getLocalTime(r)
		err := database.DB.QueryRow("SELECT reward FROM goals WHERE id = $1 AND user_id = $2 AND completed = 1 AND claimed = 0", id, userID).Scan(&reward)
		if err == nil {
			_, _ = database.DB.Exec("UPDATE goals SET claimed = 1, claimed_at = $1 WHERE id = $2 AND user_id = $3", localDate, id, userID)
			_, _ = database.DB.Exec("UPDATE user_stats SET total_coins = total_coins + $1 WHERE user_id = $2", reward, userID)
		}
	} else if r.Method == "DELETE" {
		_, _ = database.DB.Exec("DELETE FROM goals WHERE id = $1 AND user_id = $2", id, userID)
	}
