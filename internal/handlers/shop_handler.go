...
func renderShop(w http.ResponseWriter, message string) {
	var totalCoins int
	_ = database.DB.QueryRow("SELECT total_coins FROM user_stats WHERE id = 1").Scan(&totalCoins)

	rows, err := database.DB.Query("SELECT id, category, name, cost, owned FROM shop_catalog ORDER BY category ASC, id ASC")
	if err != nil {
...
func HandleShop(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		_ = r.ParseForm()
		handleShopCreate(w, category, r.FormValue("name"), r.FormValue("cost"))
		return
	case http.MethodDelete:
		if id != "" {
			_, _ = database.DB.Exec("DELETE FROM shop_catalog WHERE id = ?", id)
		}
		renderShop(w, "")
		return
	default:
...
func handleShopCreate(w http.ResponseWriter, category, name, costStr string) {
	cost, err := strconv.Atoi(costStr)
	if err != nil || strings.TrimSpace(name) == "" {
		renderShop(w, "Invalid shop entry.")
		return
	}
	...
	_, err = database.DB.Exec(
		`INSERT INTO shop_catalog (category, name, cost, owned) VALUES (?, ?, ?, 0)`,
		category, strings.TrimSpace(name), cost,
	)
	if err != nil {
		renderShop(w, "Could not add shop item.")
		return
	}

	renderShop(w, "Shop updated.")
}

func handleShopBuy(w http.ResponseWriter, id string) {
	if strings.TrimSpace(id) == "" {
		renderShop(w, "Invalid item.")
		return
	}

	tx, err := database.DB.Begin()
	if err != nil {
		renderShop(w, "Transaction failed.")
		return
	}
	defer tx.Rollback()
	...
	var cost int
	var owned int
	err = tx.QueryRow(`SELECT cost, owned FROM shop_catalog WHERE id = ?`, id).Scan(&cost, &owned)
	if err != nil {
		renderShop(w, "Item not found.")
		return
	}

	var coins int
	err = tx.QueryRow(`SELECT total_coins FROM user_stats WHERE id = 1`).Scan(&coins)
	if err != nil {
		renderShop(w, "Coin account not found.")
		return
	}

	if coins < cost {
		renderShop(w, "Not enough coins.")
		return
	}

	_, err = tx.Exec(`UPDATE user_stats SET total_coins = total_coins - ? WHERE id = 1`, cost)
	if err != nil {
		renderShop(w, "Failed to deduct coins.")
		return
	}

	_, err = tx.Exec(`UPDATE shop_catalog SET owned = owned + 1 WHERE id = ?`, id)
	if err != nil {
		renderShop(w, "Failed to update ownership.")
		return
	}

	if err = tx.Commit(); err != nil {
		renderShop(w, "Purchase could not be completed.")
		return
	}

	renderShop(w, fmt.Sprintf("Purchased %s.", id))
}
